...
const syncAndGetStudentProgress = async (courseId: string, studentId: string) => {
  const courseClasses = await ClassModel.find({ course: courseId }).select('_id');
  const classIds = courseClasses.map(c => c._id);
  const totalConductedClasses = classIds.length;
  ...
  const attendanceRate = totalConductedClasses > 0 ? (presentRecords / totalConductedClasses) * 100 : 0;

  const allTasks = await TaskModel.find({ course: courseId });
  const submissions = await SubmissionModel.find({ course: courseId, student: studentId });

  const markedSubmissions = submissions.filter((s) => s.isMarked);
  const avgGrade = markedSubmissions.length > 0 
    ? (markedSubmissions.reduce((acc, curr) => acc + (curr.marks || 0), 0) / markedSubmissions.length) 
    : 0;

  const homeworkCompletedRate = allTasks.length > 0 ? (submissions.length / allTasks.length) * 100 : 0;

  const now = new Date();
  const overdueTasksCount = allTasks.filter((task) => {
    const endDateTime = new Date(`${task.endDate}T${task.endTime}`);
    return now > endDateTime && !submissions.some(s => s.task.toString() === task._id.toString());
  }).length;
  const overdueRate = allTasks.length > 0 ? (overdueTasksCount / allTasks.length) * 100 : 0;

  let status: "on track" | "behind" | "attention" | "critical" = "on track";
  const isFreshStart = totalConductedClasses === 0 && overdueTasksCount === 0;

  if (!isFreshStart) {
    if (attendanceRate < 70 || (markedSubmissions.length > 0 && avgGrade < 40)) status = "critical";
    else if (attendanceRate < 80 || overdueTasksCount >= 3) status = "attention";
    else if (attendanceRate < 90 || overdueTasksCount >= 1) status = "behind";
  }

  return await StudentProgressModel.findOneAndUpdate(
    { course: courseId, student: studentId },
    {
      status,
      attendanceRate: parseFloat(attendanceRate.toFixed(2)),
...
const getCourseDashboardOverview = async (courseId: string) => {
  const course = await CourseModel.findById(courseId).select('students');
  if (!course) throw new Error("Course not found");
  ...
  let onTrack = 0, attention = 0, behind = 0, critical = 0;

  enrolledStudentIds.forEach((id) => {
    const progress = progressRecords.find(p => p.student.toString() === id);
    if (progress) {
      const isFresh = progress.attendanceRate === 0 && progress.avgGrade === 0 && progress.overdueRate === 0;
      if (isFresh) onTrack++;
      else {
        if (progress.status === 'on track') onTrack++;
        else if (progress.status === 'attention') attention++;
  ...
  return { onTrack, attention, behind, critical, totalStudents: enrolledStudentIds.length };
};
...
const getStudentListWithStatus = async (courseId: string, searchTerm: string = '') => {
    ...
    const studentList = validStudents.map((student) => {
        ...
        return {
            ...
            overdueRate: progress?.overdueRate || 0,
            updatedAt: progress?.updatedAt || new Date()
        };
    });

    return {
        instructorInfo: { teacher: course.teacherId || null, assistant: course.assistantId || null },
        studentList
    };
};
...
const getOverallCourseAcademicStats = async (courseId: string) => {
  ...
  const totalPotential = classIds.length * totalStudents;
  const presentCount = await AttendanceModel.countDocuments({ class: { $in: classIds }, status: { $in: ['on time', 'late'] } });

  const allTasks = await TaskModel.find({ course: courseId });
  const allSubmissions = await SubmissionModel.find({ course: courseId });
  const potentialSubmissions = allTasks.length * totalStudents;
  ...
  return {
    totalEnrolled: totalStudents,
    attendanceRate: totalPotential > 0 ? parseFloat(((presentCount / totalPotential) * 100).toFixed(2)) : 0,
    homeworkRate: potentialSubmissions > 0 ? parseFloat(((allSubmissions.length / potentialSubmissions) * 100).toFixed(2)) : 0,
    avgGrade: markedSubmissions.length > 0 ? parseFloat((totalMarks / markedSubmissions.length).toFixed(2)) : 0,
    overdueRate: 0 // logic simplified to match existing response
  };
};
...
const getDetailedTabularReport = async (courseId: string, searchTerm: string = '') => {
  ...
  const classes = await ClassModel.find({ course: courseId }).select('_id');
  ...
  const allAttendance = await AttendanceModel.find({ class: { $in: classIds } });
  const allSubmissions = await SubmissionModel.find({ course: courseId });

  return (course.students as any[]).map((student) => {
    const sId = student._id.toString();
    const attCount = allAttendance.filter(a => a.student.toString() === sId && (a.status === 'on time' || a.status === 'late')).length;
    const studentHwCount = allSubmissions.filter(s => s.student.toString() === sId && homeworkTasks.some(h => h._id.toString() === s.task.toString())).length;
    const examMarks = allSubmissions.filter(s => s.student.toString() === sId && s.isMarked && examTasks.some(e => e._id.toString() === s.task.toString()));
    const avgExam = examMarks.length > 0 ? Math.round(examMarks.reduce((a, b) => a + (b.marks || 0), 0) / examMarks.length) : 0;

    return {
      studentName: student.fullName,
      attendance: `${String(attCount).padStart(2, '0')}/${String(totalClasses).padStart(2, '0')} (${totalClasses > 0 ? Math.round((attCount/totalClasses)*100) : 0}%)`,
      hwCompleted: `${String(studentHwCount).padStart(2, '0')}/${String(homeworkTasks.length).padStart(2, '0')}`,
      hwPending: String(Math.max(0, homeworkTasks.length - studentHwCount)).padStart(2, '0'),
      examGrade: `${avgExam}%`
    };
  });
};
...
const getDetailedStudentProgress = async (courseId: string, studentId: string) => {
    ...
    const course = await CourseModel.findById(courseId)
        .populate({ path: 'teacherId', select: 'fullName image contact' })
        .populate({ path: 'assistantId', select: 'fullName image contact' }).lean();

    const progressStats = await syncAndGetStudentProgress(courseId, studentId);
    
    // Missing Task Message Call
    const alertMessage = await getMissingTasksMessage(courseId, studentId, student.fullName || 'Student');
...
const getStudentDetailedAttendanceFromDB = async (courseId: string, studentId: string) => {
  ...
  const attendanceList = classes.map(cls => {
    const record = attendances.find(a => a.class?.toString() === cls._id.toString());
    return {
      className: cls.title,
      classStart: `${cls.date.toISOString().split('T')[0]} | ${cls.time}`,
      status: record ? record.status : "Not Marked"
    };
  });
...
export const ReportServices = {
  ...
  getStudentDetailedAttendanceFromDB,
  getStudentMarksHistoryFromDB,
  getChildEnrolledCoursesFromDB
};